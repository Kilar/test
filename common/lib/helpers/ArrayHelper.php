...
class ArrayHelper extends \yii\helpers\ArrayHelper
{
    ...
    /**
     * 销毁数组元素为对象的重复的对象(保留最开始键的对象元素)
     * @param array $objs
     * @return array
     * @throws ServerErrorHttpException
     */
    protected  static function objectArrayUnique(array &$objs, array $keys) 
    {
        if (!($num = count(array_filter($objs, 'is_object'))) || $num < count($objs) ) {
            throw new ServerErrorHttpException('Each element of the array must be object');
        }
        for ($i = 0 ; $i < $num - 1 ; $i++) {
            if ($i >= ($lastKey = static::getLastKey($keys))) {
                break;
            } elseif (!isset($keys[$i])) {
                continue;
            }
            for ($j = $i + 1 ; $j < $num ; $j++) {
                if($j > $lastKey){
                    ...
                    continue ;
                } elseif ($objs[$keys[$i]] == $objs[$keys[$j]]) {
                    //销毁重复的对象
                    unset($objs[$keys[$j]], $keys[$j]);
                }
            }
        }
    }
     ...
     * @param bool|string $attribute2key 把ar对象某属性作为键
     * @return array
     */
