...
{
    /**
     * @var \app\models\ParamsFilter 参数过滤器
     */
    public $filterModel;
    /**
    ...
    public function filter($event)
    {
        ...
        //用户设置
        if ($this->filterModel->isNeedToken(null)) {
        ...
        $controller->vaildator->data = $data;
        $controller->vaildator->service = $controller->service;
    }
    ...
    {
        //检查类是否存在
        $vClass = str_replace('//', '\\', "app//api//$apiVer//validators//{$id}Validator");
        $sClass = str_replace('//', '\\', "app//api//$apiVer//service//{$id}Service");
        if (!class_exists($vClass) || !class_exists($sClass)) {
            throw new ApiException(Error::FILE_NOTFOUND, '服务类不存在');
        }
        
        //实例化操作
        $controller->service = \Yii::$container->get($sClass);
        $controller->vaildator = \Yii::$container->get($vClass);
        if (!$controller->service instanceof BaseService || !$controller->vaildator instanceof BaseValidator) {
